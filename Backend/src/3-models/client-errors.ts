...
export class UserOrVacationNotFoundError extends ClientError {
    public constructor(userId: number, vacationId:number) {
        const message = `user ${userId} or vacation ${vacationId} not found.`;
        const status = StatusCode.NotFound;
        super(message, status);
    }
}
...
export class DuplicateUserOrVacationError extends ClientError {
    public constructor(userId: number, vacationId:number) {
        const message = `user ${userId} or vacation ${vacationId} duplicate.`;
        const status = StatusCode.NotFound;
        super(message, status);
    }
}
